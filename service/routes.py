...
def list_counters():
    ...
    c = [dict(name=count[0], counter=count[1]) for count in COUNTER.items()]
...
def create_counters(name):
    ...
    if name in COUNTER:
        return abort(status.HTTP_409_CONFLICT, f"{name} exists")
...
def read_counters(name):
    ...
    if name not in COUNTER:
        return abort(status.HTTP_404_NOT_FOUND, f"{name} exist")
